...
def test_gc_leader_key_namespaced_and_stable() -> None:
    from domain.locks import (
        GC_LEADER_IDENTITY,
        REALTIME_SCHEDULER_LEADER_IDENTITY,
        gc_leader_key,
    ...
    key = gc_leader_key()
    assert key == gc_leader_key(GC_LEADER_IDENTITY)
    # Stable across calls, 64-bit, and in the 0x4000000000000000 namespace.
    assert 0 <= key < 2**64
    assert key & 0xF000000000000000 == 0x4000000000000000
    # Disjoint from scheduler leadership, store gates, region locks, admission.
    assert key != scheduler_leader_key(REALTIME_SCHEDULER_LEADER_IDENTITY)
    assert key != store_gate_key("s3://weather-data/gfs/2026-07-21/00/cycle.zarr")


def test_gc_leader_key_identity_sensitive() -> None:
    from domain.locks import gc_leader_key

    assert gc_leader_key("deployment-a") != gc_leader_key("deployment-b")
