...
> **Implementation status**: Milestones 1–12 are complete and approved (see [IMPLEMENTATION_PLAN.md](IMPLEMENTATION_PLAN.md)). The live surface today is: NOAA GFS/GEFS ingestion (GRIB2 → Zarr → PostgreSQL catalog), the FastAPI `/v1` catalog / points / search / probabilities / maps / ensembles / verifications / health surface, PostGIS + Redis caching, the domain ensemble/geo math engine, and the Next.js frontend map viewer (Milestone 12). The AI downscaling, MME calibration, ECMWF/Canada providers, and advanced frontend charting are future milestones (13–18) and are **not yet implemented**.
...
├── services/
│   ├── api/                      # FastAPI core service (v1 catalog/points/search/probabilities/maps/ensembles/verifications/health, PostGIS, Redis cache)
│   ├── ingestion/                # GRIB2 downloaders, decoders, Zarr storage, PostgreSQL catalog writer (weather-ingest CLI)
│   └── frontend/                 # Next.js / React client application (map viewer, API client)
