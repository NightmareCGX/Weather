...
def test_normalize_visibility_converts_meters_to_km() -> None:
    """VIS (m) is converted to km at ingestion."""
    dataset = _dataset_with_units(
        "visibility",
        [[10000.0, 10000.0], [10000.0, 10000.0]],
        source_unit="m",
    )
    variables = (VariableSpec("visibility", "Visibility", "km", "vis"),)
    normalized = _normalize_canonical_units(dataset, variables)
    value = float(normalized["visibility"].values[0, 0])
    assert value == pytest.approx(10.0, abs=1e-9)
    assert normalized["visibility"].attrs["units"] == "km"
