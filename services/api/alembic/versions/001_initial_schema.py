...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_forecast_centers_center_id"),
        "forecast_centers",
        ["center_id"],
        unique=True,
    )
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "model_version_id", "cycle_time", name="uq_model_run_cycle"
        ),
    )
    op.create_index(
        "idx_model_runs_cycle",
        "model_runs",
        ["model_version_id", sa.text("cycle_time DESC")],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_forecast_variables_variable_code"),
        "forecast_variables",
        ["variable_code"],
        unique=True,
    )
    ...
    op.create_index(
        op.f("ix_forecast_grids_grid_code"),
        "forecast_grids",
        ["grid_code"],
        unique=True,
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["variable_id"],
            ["forecast_variables.variable_code"],
        ),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "run_id",
    ...
    op.create_index(
        "idx_forecast_products_catalog",
        "forecast_products",
        ["run_id", "variable_id", "grid_id"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_stations_station_code"), "stations", ["station_code"], unique=True
    )
    op.create_index(
    ...
    op.create_index(
        "idx_cities_geom", "cities", ["geom"], unique=False, postgresql_using="gist"
    )
    ...
    op.create_index(
        op.f("ix_ski_resorts_resort_name"), "ski_resorts", ["resort_name"], unique=False
    )
    op.create_index(
        "idx_ski_resorts_geom",
        "ski_resorts",
        ["geom"],
        unique=False,
    ...
    op.create_index(
        "idx_verification_station_time",
        "verification_observations",
        ["station_id", sa.text("valid_time DESC")],
        unique=False,
    )
    ...
    op.create_index(
        "idx_point_query_fallback_expires",
        "point_query_fallback_audit",
        ["expires_at"],
        unique=False,
    )
...
def downgrade() -> None:
    op.drop_index(
        "idx_point_query_fallback_expires", table_name="point_query_fallback_audit"
    )
    op.drop_table("point_query_fallback_audit")
    op.drop_index(
        "idx_verification_station_time", table_name="verification_observations"
    )
    op.drop_table("verification_observations")
    op.drop_index(
        "idx_ski_resorts_geom", table_name="ski_resorts", postgresql_using="gist"
    )
    ...
    op.drop_index(
        op.f("ix_forecast_variables_variable_code"), table_name="forecast_variables"
    )
