...
- **Example Response (distribution)**:
  ...
  ```
- **`histogram_members` / `histogram_stored` (distribution delivery)**: `histogram_members` counts the raw member values and is delivered **only** with `ENSEMBLE_DUAL_SOURCE_ENABLED=true`, alongside `histogram_stored` evaluated over the **same edges** — both shaped `{edges: [...], counts: [...]}` and both counting members. They exist because the front end moved from drawing distributions out of raw members to drawing them out of the stored fields, and a change of source that shows only one line cannot be audited: a difference between the two would be indistinguishable from a rendering change (see `docs/investigations/numeric-encoding/IMPLEMENTATION.md` §19.14 for the acceptance comparison those two lines produced).
- **`histogram_stored` alone means the members are gone.** When a store's member shards have been reclaimed (`ENSEMBLE_AGGREGATE_SUPERSESSION_ENABLED`, §19.15), the response carries `histogram_stored` **without** `histogram_members` and without `members`, and the histogram's `edges` then come from the stored distribution itself — the outermost stored quantile levels, or the mean and spread of the bin encoding — rather than from member values that no longer exist. The two states are distinguishable by which fields are present: members plus two histograms is the comparison, a stored histogram alone is the answer. The stored line is `null` when the store has no usable aggregate for the variable and lead, rather than being sent as zeros — an empty distribution and an absent one are different statements. Delivery only: neither field is part of the statistics contract and neither is stored.
