...
- **Example Response (distribution)**:
  ...
  ```
- **`histogram_members` / `histogram_stored` (migration delivery, off by default)**: with `ENSEMBLE_DUAL_SOURCE_ENABLED=true` the response also carries two histograms on **one shared bin grid** — `histogram_members` counts the raw member values and `histogram_stored` evaluates the stored fields' distribution over the same edges, both shaped `{edges: [...], counts: [...]}` and both counting members. They exist because the front end is moving from drawing distributions out of raw members to drawing them out of the stored fields, and a change of source that shows only one line cannot be audited: a difference between the two would be indistinguishable from a rendering change. The stored line is `null` when the store has no usable aggregate for the variable and lead, rather than being sent as zeros — an empty distribution and an absent one are different statements. Delivery only: neither field is part of the statistics contract, neither is stored, and both disappear with the members.
