...
* **Local Docker Compose**: `docker compose up -d` executes a one-shot `weather_minio_init` container (`amazon/aws-cli`) that idempotently provisions `weather-data` as soon as `weather_minio` healthcheck passes.
